...
| `github-actions-apply` | the `apply` matrix job and `destroy` | `repo:<repo>:environment:<GitHubEnvironment>`, exactly one per account (`management` in the management account) | `AdministratorAccess` capped by the `github-actions-apply-boundary` permissions boundary (denies Identity Center, CloudTrail changes, organization destruction, switching off the security services, edits to the boundary, the `github-actions-*`, `platform-*` and `terraform-*` roles, the OIDC provider, the state bucket's settings and the bootstrap stack; also `organizations:*` / `account:*` everywhere except management). The role itself is still `AdministratorAccess`: narrowing it to the services actually used is the open half of PLAN 3.4 |
