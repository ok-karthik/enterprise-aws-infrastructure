...
Rolls the Day-0 bootstrap template (`infrastructure-bootstrap/cloudformation/account-bootstrap.yaml`: state bucket, GitHub OIDC provider, `github-actions-plan` / `github-actions-apply` roles and boundary) out to **every member account** in the targeted OUs, with no human step when a new account joins. Service-managed StackSets with auto-deployment on; stacks are retained if an account leaves or an instance is removed.
...
- **The template body is an input**, so the module has no file paths. The live leaf (`infrastructure-live/_global/governance/bootstrap-stacksets`) passes `file(...)`.
- **Prerequisites** (done by `infrastructure-bootstrap/bootstrap.sh`): AWS Organizations with all features, and StackSets trusted access enabled. Applied from the management account, by the owner.
...
| <a name="input_template_body"></a> [template\_body](#input\_template\_body) | Body of the account-bootstrap CloudFormation template. The live leaf reads infrastructure-bootstrap/cloudformation/account-bootstrap.yaml, so this module stays free of file paths. | `string` | n/a | yes |
